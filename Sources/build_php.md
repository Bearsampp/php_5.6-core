...
1. Get new PHP sources: `php-5.6.40-src.zip`
...
3. Open a command prompt and enter the build directory: `cd c:\php-sdk\`
4. Run the buildtree batch script to create the required directory structure: `bin\phpsdk_buildtree.bat phpdev`
5. Copy `C:\php-sdk\phpdev\vc9` to `C:\php-sdk\phpdev\vc11`
6. Extract PHP source code to `C:\php-sdk\phpdev\vc11\xXX`
    * It will create a sub-directory, for example: `C:\php-sdk\phpdev\vc11\x86\php-5.6.40-src`
7. Copy `phpdev\vc11\noarch` from this repo to `C:\php-sdk\phpdev\vc11\xXX`
8. Extract dependency libraries for building PHP:
    * Extract `deps-5.6-vc11-xXX.7z` into `C:\php-sdk\phpdev\vc11\xXX\`
9. Copy the following BAT files from this repo to `C:\php-sdk\phpdev\vc11\xXX\`:
```
# for x86:
...
1. Copy PHP patch `phpdev\vc11\noarch\php-5.6.40.patch` to `phpdev\vc11\xXX\`
...
4. Apply the patch: `patch.exe -p0 -u <php-5.6.40.patch`
...
1. Add `/D_USING_V110_SDK71_` directive to CFLAGS_PHP in `C:\php-sdk\phpdev\vc11\xXX\php-5.6.40-src\win32\build\config.w32`:
```
DEFINE("CFLAGS_PHP", "/D_USING_V110_SDK71_ /D _USRDLL /D PHP5DLLTS_EXPORTS /D PHP_EXPORTS \
```
2. For **x86**: add `/SUBSYSTEM:CONSOLE,5.01` directive to LDFLAGS in `C:\php-sdk\phpdev\vc11\x86\php-5.6.40-src\win32\build\config.w32`:
```
ADD_FLAG("LDFLAGS", '/SUBSYSTEM:CONSOLE,5.01 /libpath:"' + php_usual_lib_suspects + '" ');
```
3. For **x64**: add `/SUBSYSTEM:CONSOLE,5.02` directive to LDFLAGS in `C:\php-sdk\phpdev\vc11\x64\php-5.6.40-src\win32\build\config.w32`:
```
ADD_FLAG("LDFLAGS", '/SUBSYSTEM:CONSOLE,5.02 /libpath:"' + php_usual_lib_suspects + '" ');
```

#### Change target version
Change target Windows version in `C:\php-sdk\phpdev\vc11\xXX\php-5.6.40-src\win32\build\config.w32.h.in`:
```
...
#### select.c
Rollback changes in `C:\php-sdk\phpdev\vc11\xXX\php-5.6.40-src\win32\select.c`:
...
#### inet.c/h
Extract files `win32\inet.h` and `win32\inet.c` from old PHP sources (5.4.9) to `C:\php-sdk\phpdev\vc11\xXX\php-5.6.40-src\win32\` (overwrite existing files).

#### info.c
Remove next lines from `C:\php-sdk\phpdev\vc11\xXX\php-5.6.40-src\ext\standard\info.c`:
```
...
#### php_sockets.h
Add code to `C:\php-sdk\phpdev\vc11\xXX\php-5.6.40-src\ext\sockets\php_sockets.h`:
...
#### sockets.c
Add code to `C:\php-sdk\phpdev\vc11\xXX\php-5.6.40-src\ext\sockets\sockets.c`:
```
#if (_WIN32_WINNT < 0x0600)
WINAPI if_nametoindex (__in PCSTR iface)
{
...
#### tls_var.c/h
1. Copy supplied `tls_var.c` and `tls_var.h` from `phpdev\vc11\noarch\php-5.6.40-src\main` to `C:\php-sdk\phpdev\vc11\xXX\php-5.6.40-src\main`
2. Modify file `C:\php-sdk\phpdev\vc11\xXX\php-5.6.40-src\win32\build\config.w32`:
```
# old:
...
Change file `C:\php-sdk\phpdev\vc11\xXX\php-5.6.40-src\main\php_open_temporary_file.c`:
...
#### zend_execute_API.c
Change file `C:\php-sdk\phpdev\vc11\xXX\php-5.6.40-src\Zend\zend_execute_API.c`:
1. Add lines to the top of file (after all `#include` blocks):
```
#if defined(ZTS) && defined(PHP_WIN32)
...
3. Destination folder for binaries:
  * For **x86**: `C:\php-sdk\phpdev\vc11\x86\php-5.6.40-src\Release_TS`
  * For **x64**: `C:\php-sdk\phpdev\vc11\x64\php-5.6.40-src\x64\Release_TS`
